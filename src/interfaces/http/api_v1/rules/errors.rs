...
use rve_core::{
  application::{RuleCommandServiceError, RuleQueryServiceError},
  domain::common::RuleId,
  ports::rule_repository::RuleRepositoryError,
};
use serde::Serialize;
...
pub(super) fn map_rule_query_error(error: RuleQueryServiceError) -> ApiError {
  match error {
    RuleQueryServiceError::Repository(error) => map_repository_error(error),
  }
}

pub(super) fn map_rule_command_error(error: RuleCommandServiceError) -> ApiError {
  match error {
    RuleCommandServiceError::Repository(error) => map_repository_error(error),
  }
}
