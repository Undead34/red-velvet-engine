...
use super::{
  dto::{Pagination, RuleListResponse, RuleResponse},
  errors::{ApiError, ApiResult, map_rule_query_error, parse_rule_id},
...
pub async fn list_rules(
  ...
  let page_data =
    state.rule_query_service.list_rules(page, limit).await.map_err(map_rule_query_error)?;

  Ok(Json(RuleListResponse::from_rules(page_data.items, page, limit, page_data.total)))
}
...
pub async fn get_rule(
  ...
  Ok((headers, Json(RuleResponse::from(&rule))))
}
