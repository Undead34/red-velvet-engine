...
mod mapper {
  ...
  pub fn build_message(event: &Event) -> Result<dataflow_rs::Message, RuntimeEngineError> {
    let event_json = event_to_runtime_json(event)?;
  ...
  fn event_to_runtime_json(event: &Event) -> Result<Value, RuntimeEngineError> {
    let mut event_json = serde_json::to_value(event)
      .map_err(|err| RuntimeEngineError::Internal { message: err.to_string() })?;

    if let Some(value_transfer) = event.payload.as_value_transfer() {
      let minor_units = i64::try_from(value_transfer.money.amount().units()).map_err(|_| {
        RuntimeEngineError::Internal {
          message: "event payload money minor units exceed JSON i64 range".to_owned(),
        }
      })?;

      if let Some(payload) = event_json.get_mut("payload").and_then(Value::as_object_mut) {
        payload.insert(
          "money".to_owned(),
          json!({
            "minor_units": minor_units,
            "ccy": value_transfer.money.currency().alpha(),
          }),
        );
      }
    }
